...
def Analyse_modale(Nx, Ny, dx, dy, Lx, Ly, Es, Is, rho_s,rho_c, Ss):
    ...
    Mc,Kc,Mcouplage=ballotement.matrices_2D(Lx, Ly, Nx, Ny, dx, dy)
    ...
    W_tri=np.zeros((Ns+Nx-4,2),dtype="float64")
    
    V_reel=np.zeros((Ns+NE-4,Ns+Nx-4),dtype="float64")
    ...
    for i in range(len(W)):
        if abs(W[i].real)<10e-1:
            W[i]=0.0 #filtrage
        if W[i].real != np.inf and W[i].real > 0.1:
    ...
    print("Fréquences de résonnance ballotement+poutre (10 premières) : \n ", W_tri2[:10])
...
def graph(W_tri2, V_reel2, Lx, Ly, Nx, Ny, compteur):
    ...
    for j in range(compteur-1):
        V_poutre=V_reel2[:Ns,j]/np.linalg.norm(V_reel2[:Ns,j])
        for k in range(N):
            for i in range(nbr_x):
                X_plt[k*(nbr_x-1)+i]=k*dx+x[i];#liste des abscisses des différents points
                # Def_modale[k*(nbr_x-1)+i,j]=V_reel2[2*k,j]*Poutre_flexion.N1(x[i],dx)+V_reel2[2*k+1,j]*Poutre_flexion.N2(x[i],dx)+V_reel2[2*k+2,j]*Poutre_flexion.N3(x[i],dx)+V_reel2[2*k+3,j]*Poutre_flexion.N4(x[i],dx)
                ...
                # Def_modale[k*(nbr_x-1)+i,j]=V_poutre[2*k]*Poutre_flexion.N1(x[i],dx)+V_poutre[2*k+1]*Poutre_flexion.N2(x[i],dx)+V_poutre[2*k+2]*Poutre_flexion.N3(x[i],dx)+V_poutre[2*k+3]*Poutre_flexion.N4(x[i],dx)
                Def_modale[k*(nbr_x-1)+i,j]=np.sign(V_poutre[2*k])*Poutre_flexion.N1(x[i],dx)+V_poutre[2*k+1]*Poutre_flexion.N2(x[i],dx)+np.sign(V_poutre[2*k+2])*Poutre_flexion.N3(x[i],dx)+V_poutre[2*k+3]*Poutre_flexion.N4(x[i],dx)
    
    
    fig, axs = plt.subplots(4, 2,sharex='col', figsize=(10,10), gridspec_kw={'height_ratios': [2,1,2,1]})
    
    V_reel_max=np.max(V_reel2[Ns:,Aff_min:Aff_max])*1.1
    V_reel_min=np.min(V_reel2[Ns:,Aff_min:Aff_max])*1.1
    ...
    for i in range(0,Aff_max-Aff_min,2):
        for p in range(2):
            ...
            for k in range(Ny):
                for j in range(Nx):
                    Z[k,j]=V_reel2[Ns+Nx*k+j,i+p+Aff_min]/np.linalg.norm(V_reel2[Ns:,i+p+Aff_min])
            
            c=axs[i,p].contourf(X,Y,Z,100, cmap='jet', vmin=V_reel_min, vmax=V_reel_max, extend='neither')
            axs[i,p].set_ylabel('y(m)')
            axs[i,p].set_xlabel('x(m)')
            axs[i,p].set_title('Pressions (relatives) - Mode {}'.format(i+p+Aff_min+1)) 
            #fig.colorbar(c, ax=axs[i+1,p])        
            
            
            axs[i+1,p].plot(X_plt,Def_modale[:,i+p+Aff_min],color='red')
            # axs[i,p].plot(x_graph,V_reel[:Ns:2,i+p+Aff_min],color='red')
            axs[i+1,p].set_yticks([0])
            axs[i+1,p].grid(True)
            axs[i+1,p].set(title='Déplacements - Mode {}'.format(i+p+Aff_min+1))  
...
Ny=7#points de discrétisation selon y
...
W,V,compteur=Analyse_modale(Nx, Ny, dx, dy, Lx, Ly, Es, Is, rho_s,rho_c,Ss )
